...
def _run_periodic_actions(failed: list):
    """
    Runs in maintenance job (daily 07:05 BKK).

    borrow_health : daily health check — close early if health < 1.5x
    aero_vote EXIT: exit expired locks only — ENTER is handled by plan system (dispatcher)
    megapot/deploy: handled by plan system (dispatcher._add_periodic_actions)
    """
    # ── aero_vote EXIT only ────────────────────────────────────────────────────
...
def _open_platform_with_recovery_inner(platform_key: str, wid: str):
    ...
    if platform_key == 'aero_vote':
        ...
        _ve = executor.w3.eth.contract(address=_VE_ADDR, abi=_VE_BAL_ABI)
        # On-chain guard: abort if wallet already holds veAERO NFT(s)
        try:
            onchain_nfts = _ve.functions.balanceOf(executor.WALLET).call()
        except Exception as _ce:
            log.warning(f'aero_vote: balanceOf failed: {_ce} — aborting enter to be safe')
            _action_log(platform_key, 'fail', f'balanceOf error: {_ce}')
            return
        if onchain_nfts > 0:
            # Scan and reconcile orphan(s) into DB
            log.warning(f'aero_vote: wallet has {onchain_nfts} veAERO NFT(s) on-chain — reconciling before enter')
            ...
            reconciled = 0
            for tid in range(max(1, anchor - 500), anchor + 500):
                try:
                    owner = _ve.functions.ownerOf(tid).call()
                    if owner.lower() == wallet_lower:
                        locked = _ve.functions.locked(tid).call()
                        aero = locked[0] / 1e18
                        if aero > 0:
                            unlock = date.fromtimestamp(locked[1]).isoformat() if locked[1] else ''
                            existing = _sqlite3.connect(state.DB_PATH).execute(
                                "SELECT id FROM positions WHERE platform='aero_vote' AND status='active' AND amount_wei LIKE ?", (f'{tid}|%',)
                            ).fetchone()
                            if not existing:
                                with _sqlite3.connect(state.DB_PATH) as _c:
                                    _c.execute(
                                        ...
                                        ('aero_vote','AERO',f'{tid}|{int(aero*1e18)}',date.today().isoformat(),unlock,'','active'),
                                    )
                                log.warning(f'aero_vote: reconciled orphan tokenId={tid} aero={aero:.4f} unlock={unlock}')
                                reconciled += 1
                except Exception:
                    pass
            if reconciled or state.get_active('aero_vote'):
                log.warning('aero_vote: active NFT exists on-chain — skipping enter')
                _action_log(platform_key, 'fail', f'skipped: {onchain_nfts} NFT(s) already on-chain')
                return
        try:
            executor._local_nonce = None
            ...
            if not executor.DRY_RUN:
                ...
                with _sqlite3.connect(state.DB_PATH) as _c:
                    _c.execute(
                        'INSERT INTO positions (platform,token,amount_wei,entry_date,expiry_date,tx_hash,status) VALUES (?,?,?,?,?,?,?)',
                        ('aero_vote','AERO',f'{token_id}|{aero_wei}',date.today().isoformat(),lock_end_str,result['tx_lock'],'active'),
                    )
                state.log_daily_stat('vote')
                log.info(f'aero_vote enter done  tokenId={token_id}  lock_end={lock_end_str}')
                _action_log(platform_key, 'ok', f'tokenId={token_id}  unlock={lock_end_str}')
            else:
                log.info('[DRY RUN] aero_vote_enter done (no DB write)')
                _action_log(platform_key, 'ok', 'DRY RUN')
        except Exception as _e:
            log.error(f'aero_vote enter failed: {_e}')
            _action_log(platform_key, 'fail', str(_e))
        return
