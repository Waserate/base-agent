"""
roundtrip_uni_lp.py — Live test: mint + pause + close for Uniswap v3 LP pools.

Flow per pool:
  1. Acquire token0 + token1 via swap (50:50 USD, $5 total)
  2. Mint full-range position → get tokenId
  3. PAUSE — แจ้งผลแล้วรอ user กด Enter
  4. decreaseLiquidity + collect → token0 + token1 in wallet
  5. Convert back to ETH → report net ETH delta
  6. Auto-advance to next pool

Usage:
    python roundtrip_uni_lp.py                          # all 10 pools
    python roundtrip_uni_lp.py uni_lp_weth_usdc_3000    # single pool
"""
...
import state, executor, swap, uni_lp
...
def roundtrip(pool_key: str) -> bool:
    ...
    # ── STEP 4: PAUSE — รอ user สั่งถอน ───────────────────────────────────
    log.info(f'\n{"*" * 60}')
    log.info(f'DEPOSIT DONE — pool={pool_key}')
    log.info(f'  tokenId : {token_id}')
    log.info(f'  tx      : {txh}')
    log.info(f'  Check on-chain แล้วกด Enter เพื่อถอน (Ctrl+C เพื่อหยุด)')
    log.info(f'{"*" * 60}')
    try:
        input('\n>>> กด Enter เพื่อ WITHDRAW ... ')
    ...
    try:
        txh_close = uni_lp.close_uni_lp(token_id)
...
if __name__ == '__main__':
    ...
    pools = CFG.get('phase_uni_lp', [])
    if len(sys.argv) > 1:
        pools = [sys.argv[1]]

    print(f'\nUNI_LP ROUNDTRIP TEST — {len(pools)} pools')
